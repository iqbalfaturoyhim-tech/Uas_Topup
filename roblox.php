...
/* BACKGROUND PARTICLES (SAMA DENGAN ML) */
.bg span{
  position:absolute;
  width:6px;
  height:6px;
  background:#00f6ff;
  box-shadow:0 0 15px #4f8527ff;
  border-radius:50%;
...
.box{
  ...
  background:rgba(227, 33, 33, 0.65);
  padding:30px 35px;
  width:420px;
  border-radius:20px;
  border:2px solid #00f6ff;
  box-shadow:0 0 25px #e6801aff,0 0 45px #ffa077ff;
...
<form method="POST">
<input type="text" name="id_game" placeholder="ID Game Roblox" required>
<input type="text" name="id_server" placeholder="ID Server (jika ada)" required>
<input type="text" name="username_game" placeholder="Username Roblox" required>

<select name="jumlah" required>
<?php foreach($robux_list as $r): ?>
<option value="<?= $r ?>"><?= number_format($r) ?> Robux</option>
<?php endforeach; ?>
</select>
...
<a href="../dashboard.php" style="display:block;text-align:center;margin-top:10px;color:#00f6ff;text-decoration:none;font-weight:bold;">
...
if(isset($_POST['beli'])){
  ...
  VALUES
  ('$invoice','$_SESSION[username]','Roblox $jumlah Robux','$jumlah','$total',
...
<div class="invoice-wrapper">
  <div class="invoice" id="invoice">
    ...
    <table cellpadding="6" width="100%">
      ...
      <tr><td>Produk</td><td>: <?= $jumlah ?> Robux</td></tr>
